...
EXPECTED_ATTACHMENT_IDENTITIES_ASSET = {"blobs/ea4bcf42-2b72-4903-919b-b856443fc511"}
...
EXPECTED_ATTACHMENT_IDENTITIES_EVENT = {"blobs/1718b2c9-8764-4733-8926-4688c7ad192e"}

ATTRIBUTES_WITH_CUSTOM_ATTR = {
    "arc_display_type": "Test",
...
class TestDictMerge(TestCase):
    ...
    def test_attachment_identities_assets(self):
        self.assertEqual(
            len(dictmerge.attachment_identities_assets(ASSET_WITH_ATTACHMENT)),
            1,
            msg="attachment_identities_assets returns incorrect length",
        )
        self.assertEqual(
            dictmerge.attachment_identities_assets(ASSET_WITH_ATTACHMENT),
            EXPECTED_ATTACHMENT_IDENTITIES_ASSET,
            msg="attachment_identities_assets returning incorrect value for attachment ID",
        )
    ...
    def test_attachment_identities_events(self):
        self.assertEqual(
            len(dictmerge.attachment_identities_events(EVENT_WITH_ATTACHMENT)),
            1,
            msg="attachment_identities_events returns incorrect length",
        )
        self.assertEqual(
            len(dictmerge.attachment_identities_events(EVENT_WITH_ASSET_ATTACHMENT)),
            0,
            msg="events_ext_attr counting events with asset attachments",
        )
        self.assertEqual(
            dictmerge.attachment_identities_events(EVENT_WITH_ATTACHMENT),
            EXPECTED_ATTACHMENT_IDENTITIES_EVENT,
            msg="attachment_identities_events returning incorrect value for attachment ID",
        )

    def test_level_1_sanitization(self):
        self.assertEqual(
            dictmerge.level_1_sanitization(ATTRIBUTES_NO_CUSTOM_ATTR),
            ATTRIBUTES_NO_CUSTOM_ATTR,
            msg="level_1_sanitization returns incorrect result",
        )
        self.assertNotEqual(
            dictmerge.level_1_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            ATTRIBUTES_WITH_CUSTOM_ATTR,
            msg="level_1_sanitization returns incorrect result",
        )
        self.assertEqual(
            dictmerge.level_1_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            EXPECTED_LEVEL_1,
            msg="values not properly redacted in level_1_sanitization",
        )

    def test_level_2_sanitization(self):
        self.assertNotEqual(
            dictmerge.level_2_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            ATTRIBUTES_WITH_CUSTOM_ATTR,
            msg="level_2_sanitization returns incorrect result",
        )
        self.assertRegex(
            str(dictmerge.level_2_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR).values()),
            r"#",
    ...
    def test_level_3_sanitization(self):
        self.assertNotEqual(
            dictmerge.level_3_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            ATTRIBUTES_WITH_CUSTOM_ATTR,
            msg="level_3_sanitization returns incorrect result",
        )
        self.assertRegex(
            str(dictmerge.level_3_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR).values()),
            r"#",
        ...
        self.assertEqual(
            dictmerge.level_3_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            EXPECTED_LEVEL_3,
            msg="keys and/or values not properly redacted in level_3_sanitization",
        )

    def test_level_4_sanitization(self):
        self.assertNotEqual(
            dictmerge.level_4_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            ATTRIBUTES_WITH_CUSTOM_ATTR,
            msg="level_4_sanitization returns incorrect result",
        )
        self.assertRegex(
            str(dictmerge.level_4_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR).keys()),
            r"#",
            msg="keys not properly redacted in level_4_sanitization",
        )
        self.assertRegex(
            str(dictmerge.level_4_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR).values()),
            r"#",
        ...
        self.assertEqual(
            dictmerge.level_4_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            EXPECTED_LEVEL_4,
            msg="keys and/or values not properly redacted in level_4_sanitization",
        )

    def test_level_5_sanitization(self):
        self.assertNotEqual(
            dictmerge.level_5_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            ATTRIBUTES_WITH_CUSTOM_ATTR,
        ...
        self.assertEqual(
            dictmerge.level_5_sanitization(ATTRIBUTES_WITH_CUSTOM_ATTR),
            EXPECTED_LEVEL_5,
            msg="attributes not properly redacted in level_5_sanitization",
        )
